...
def parse_args():
    ...
    parser.add_argument(
        "--no-subscribe", "-n", action='store_true',
        help="Do not subscribe/bind for BF Runtime notifications")
...
def set_paths(install_dir):
    ...
    logging.info('SDE was built with python {}'.format(ver));
...
than "count" entries.
    ...
    try:
        ec = getattr(p4_pd, "{}_get_entry_count".format(table))(sess_hdl=sess, dev_tgt=tgt)
    ...
    fh = getattr(p4_pd, "{}_get_first_entry_handle".format(table))(sess_hdl=sess, dev_tgt=tgt)
    entries = [fh]
    if ec > 1:
        entries.extend(getattr(p4_pd, "{}_get_next_entry_handles".format(table))(fh, ec-1, sess_hdl=sess, dev_tgt=tgt))
    return entries[0:ec]
...
Optional parameters:
    ...
    for e in entries:
        entry = getattr(p4_pd, "{}_get_entry".format(table))(e, read_from_hw, sess_hdl=sess, dev_id=tgt.dev_id)
        print("----------------------")
        print("Entry Handle:", e, end="")
        if get_ttl:
            ttl = getattr(p4_pd, "{}_get_tt".format(table))(e, sess_hdl=sess, dev_id=tgt.dev_id)
            if ttl == 0:
                print("Static", end="")
            else:
                print("TTL: ", ttl, end="")
    ...
    try:
        ...
        deh = getattr(p4_pd, "{}_table_get_default_entry_handle".format(table))(sess_hdl=sess, dev_tgt=tgt)
        entry = getattr(p4_pd, "{}_get_entry".format(table))(deh, read_from_hw, sess_hdl=sess, dev_id=tgt.dev_id)
        print("*********************")
...
contain a list of entries to delete (instead of all)
    ...
    for e in entries:
        getattr(p4_pd, "{}_table_delete".format(table))(e, sess_hdl=sess, dev_id=tgt.dev_id)
    conn_mgr.end_batch(True)

    try:
        getattr(p4_pd, "{}_table_reset_default_entry".format(table))(sess_hdl=sess, dev_tgt=tgt)
    except:
        pass
    try:
        getattr(p4_pd, "{}_idle_tmo_disable".format(table))(sess_hdl=sess, dev_id=tgt.dev_id)
    except:
        pass
...
def get_action_profile_entries(table, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    ...
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    ec = getattr(p4_pd, "{}_get_act_prof_entry_count".format(table))(sess_hdl=sess, dev_tgt=tgt)

    if ec == 0:
        return []

    fh = getattr(p4_pd, "{}_get_first_member".format(table))(sess_hdl=sess, dev_tgt=tgt)
    entries = [fh]
    if ec > 1:
        entries.extend(getattr(p4_pd, "{}_get_next_members".format(table))(fh, ec-1, sess_hdl=sess, dev_tgt=tgt))
...
def dump_action_profile(table, entries=None, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    ...
    for e in entries:
        entry = getattr(p4_pd, "{}_get_member".format(table))(e, read_from_hw, sess_hdl=sess, dev_id=tgt.dev_id)
        print("----------------------")
...
def get_action_selector_groups(table, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    ...
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    gc = getattr(p4_pd, "{}_get_selector_group_count".format(table))(sess_hdl=sess, dev_tgt=tgt)
    if gc == 0:
        return []

    fg = getattr(p4_pd, "{}_get_first_group".format(table))(sess_hdl=sess, dev_tgt=tgt)
    groups = [fg]
    if gc > 1:
        groups.extend(getattr(p4_pd, "{}_get_next_groups".format(table))(fg, gc-1, sess_hdl=sess, dev_tgt=tgt))
...
def get_action_selector_group_members(table, group, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    ...
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    try:
        fh = getattr(p4_pd, "{}_get_first_group_member".format(table))(group, sess_hdl=sess, dev_id=tgt.dev_id)
        entries = [fh]
        ec=1
    except:
        return []
    ...
    while True:
        try:
            more_entries = getattr(p4_pd, "{}_get_next_group_members".format(table))(group, fh, 120, sess_hdl=sess, dev_id=tgt.dev_id)
...
class ModelTime:
    def __init__(self):
        if TARGET != 'asic-model':
            logging.error("This functionality is only available on the ASIC Model")
            return None
        logging.info("Make sure you run the model with --time-disable parameter")
        self.model_time = 0
        print(self)
    ...
    def sendp_us(self, pkt=None, iface=None, count=1, us=1, verbose=False):
        ...
        if 'sendp' not in globals():
            logging.error(
                """Scapy has not been loaded. Import Scapy using:')
                   from scapy.all import *
                """)
...
def main():
    ...
    if args.scapy:
        ...
        if os.getuid() != 0:
            logging.warning(
"""
You requested importing Scapy, but are not running as root.
